...
import React,{ useReducer, useCallback, useRef, useState, useLayoutEffect, useMemo } from "react";
...
const TopPage = () => {
  const [liffState, setLiffState] = useState<LiffStateEnum>(LiffStateEnum.notApplicable);
  const [userName, setUserName] = useState('');


  const [targetWeekdays, dispatchTargetWeekdays] = useReducer<React.Reducer<WeekdayObjType[], TargetWeekdayReducerActionType>>(targetWeekdaysReducer, defalutTargetWeekdays);
  ...
  const submitHandler = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    const notifyWeekdayArray = Array.from(formRef?.current?.getElementsByTagName('input') ?? []).map(({checked}) => Number(checked));
  ...
  useLayoutEffect(() => {
    (async () => {
      await liff.init({liffId: '2000603396-QBE1npvl', withLoginOnExternalBrowser: true});
      let liffEnum = 0;
      if (liff.isLoggedIn()) {
        liffEnum += 1;
      ...
      <div>
        ...
        <form ref={setFormRef} onSubmit={submitHandler} >
          <div className={cn(styles.flexBox, styles.checkboxWrapper)}>
            {targetWeekdays.map(({id, text, value}) => <WeekdayCheckBox key={id} checked={value} id={id} onChangeDispatch={dispatchTargetWeekdays}>{text}</WeekdayCheckBox>)}
          </div>
          ...
          <button type="submit" className={styles.submitButton} disabled={liffState < LiffStateEnum.isLiff}>設定を更新</button>
          {liffState < LiffStateEnum.isLiff && <p>LIFFブラウザから開いてください</p>}
          {liffState === LiffStateEnum.login && <p>※LIFFブラウザから開かないと設定の更新はできませんが、設定を確認することはできます。</p>}
        </form>
      </div>
    )}
    </main>
